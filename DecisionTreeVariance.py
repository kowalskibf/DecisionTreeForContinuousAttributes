...
class DecisionTreeMinimizingVariance(DecisionTree):
    def find_best_attribute(self, attributes, records):
        ...
        for attribute in attributes:
            ...
            if attribute.continuous:
                j=1
                for split_point in (sps:=find_split_points_uniform_min_variance(attribute, self.result_attribute, records)):
                    if not j%10:
    ...
    def build_tree_recurr(self, node:Node, attributes_left:list, records_left:list):
        ...
        best_attribute, split_point = self.find_best_attribute_by_minimizing_variance(attributes_left, records_left)
    ...
    def build_tree(self):
        ...
        best_attribute, split_point = self.find_best_attribute_by_minimizing_variance(attributes_left, self.records)
    ...
    def find_best_attribute_by_minimizing_variance(self, attributes, records):
        if len(attributes) == 0:
            return None
        max_gain = float("-inf")
        max_atr = None
